...
import { getProfile, postOffer } from '../../contexts/apis';
...
export default function OfferForm() {
    ...
    const addIdentifier  = (event) => {
        if(event.keyCode === 13) {
            ...
            setOfferForm({...offerForm, 
                identifiers: [...offerForm.identifiers, identifier]});
        }
    }
    ...
    const handleTitle = (value) => setOfferForm({...offerForm, title: value});
    const handleLocation = (value) => setOfferForm({...offerForm, location: value});
    const handleDescription = (value) => setOfferForm({...offerForm, description: value});

    useEffect(() => {
        setIdentifier('');
        setIdentifierText(offerForm.identifiers.join(' '));
    }, [offerForm.identifiers]);
    ...
    const submitOffer = () => {
        postOffer({...offerForm,
            ...
            phone: profile.phone, 
            group: profile.group},
            successPostOffer, failurePostOffer);
    };

    const successPostOffer = () => {
        ...
        <div className="flex flex-col request-container p-4 items-center">
            ...
            <div className='mb-4 mt-2 sm:w-full md:w-3/4 text-sm md:text-base'>
                <input className="rounded-md border-[1px] border-black pb-2 pl-2 w-full 
                    ...
                    placeholder='Offer title: e.g. Need Accomodation - Galati or Braila county' 
            ...
            <div className='mb-4 mt-2 sm:w-full md:w-3/4 text-sm md:text-base'>
                <textarea className="rounded-md border-[1px] border-black pb-2 pl-2 
                    w-full request-form-input" type="text" id="username" name="username" 
                    placeholder='Offer description and additional information' 
            ...
            <div>
                <button className='bg-blue-500 hover:bg-blue-700 text-white 
                    font-bold py-2 px-4 rounded mt-4' onClick={submitOffer}>Submit</button>
            </div>
        </div>
    );
}